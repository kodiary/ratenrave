...
// not from submit, so load data
            $data['title'] = "College Manager";
            //removeexp();
            $data['cuisine_list'] = faculties();
            $data['resturant'] = \App\Http\Models\Products::find(($id > 0) ? $id : \Session::get('session_restaurant_id'));
            $data["route"] = \Route::getCurrentRoute()->getPath();
            
            return view('dashboard.college.info', $data);
        }
    }
